...
- **Turns are judged by the LLM** in `internal/game/llm.go` (`openAIEvaluator`, OpenAI-compatible: Yandex Cloud / DeepSeek). `POST /api/game/attempt {game_key, character_key, transcript:[{choice,reply,options}], choice, anger}` → `{reply, art, achieved, game_over, anger, options[]}` (`choice:""` = opening turn). The transcript is trimmed to the **history budget** — `historyTokens` 6000 / `historyExchanges` 12, whichever binds first — and older exchanges are forgotten. (`modelContextTokens` minus `outputReserveTokens` is the outer bound, but at a 1M-token window it never binds.)
