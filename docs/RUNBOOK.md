...
- **`source`** says where the geometry came from and is set by whatever wrote the
  ...
  floor a fresh database installs for itself; `generated` is an admin having
  pressed «пересобрать». **`daily` never appears in this table** — the nightly
  floor is derived rather than stored, so seeing it on the admin read and *not*
  here is correct.
...
- **One row after several restarts is correct.** The starting floor is installed
  when the table is *empty*, not on every boot — and **the nightly rebuild writes
  nothing at all**, so a table that has not grown in a fortnight is healthy rather
  than stuck.
- **The newest row is not necessarily the floor in force.** A row is an *override*
  and it expires at the next 21:00 UTC, so a row whose `created_at` is older than
  the last boundary is history. Read the next section.

**What to conclude when they disagree.** The served id is the floor the process is
running; the newest row is what a restart *would* start from. They differ in three
ways, and only one of them is a problem:

- **the row is newer than the served floor** — somebody rebuilt the office by hand
  (`source = generated`) or inserted a row directly, and the process has adopted it
  already if the log line below is there;
- **the served floor has no row at all** (`source = daily` on the admin read) — the
  ordinary state after any 21:00, because the day's office is derived rather than
  stored;
- **the row is from today and the process is serving something else** — that is the
  one worth investigating, and the boot log says why:

```bash
ssh psycho "sudo journalctl -u psycho-space -n 200 --no-pager | grep -i gamefintech"
```

- `gamefintech: stored layout is not playable, using the day's own` — the stored
  row **failed validation** and was refused rather than served, so the office is
  the date's own floor and the row is still sitting in the table. The log line
  names the first problem and the index of the object at fault (`too_close`,
  `off_floor`, `split_floor`, `spot_blocked`, `bad_size`, `bad_kind`, `bad_window`,
  `too_many`). This is the intended failure: a floor the single-pass resolver
  cannot survive would strand bodies inside furniture or push them through a wall.
- `gamefintech: the stored floor belongs to an earlier day, using the day's own` —
  **not a fault.** The override expired at a 21:00 that has since passed, and this
  is what every boot after the first day of an untouched database logs.
...
- `gamefintech: the day's office could not be drawn, ...` — the generator refused
  the date's own seed. No date in a five-year sweep does this, so it means the
  generator itself changed; the office keeps whatever floor it had (or the starting
  one, at boot) and nothing is evicted.
- The process **refuses to boot** only if reading the table failed outright, which
  is a database problem rather than a game one.

#### The office rebuilds itself every night

**At 21:00 UTC — midnight Moscow — the floor is redrawn and everybody in the office
is thrown out with the «РЕМОНТ» ending** ([ADR-062](adrs/ADR-062-the-days-office-is-a-function-of-the-day.md)).
It is a rule of the game, stated on the splash screen, and it is the answer to
"why did the desks move overnight" and "why did my shift end at midnight".

**Nothing is scheduled and nothing is written.** The day's office is a pure
function of the date — `Generate(seedForDay(renovationDay(now)))` — and the game's
own 20 Hz simulation tick is what notices the boundary has passed. So there is no
cron, no timer goroutine and no row: the table is only ever written by a fresh
database installing the starting floor and by an admin rebuilding by hand.

**The one line to grep is the same one a manual rebuild produces**, distinguished
by its `source`:

```bash
ssh psycho "sudo journalctl -u psycho-space --since '21:00' --no-pager | grep -i 'office renovated'"
```

```
gamefintech: office renovated   layout_id=… source=daily      ended=2   ← the nightly rebuild
gamefintech: office renovated   layout_id=… source=generated  ended=2   ← somebody pressed the button
```

`ended` is how many shifts it finished. A nightly line with `ended=0` is the normal
case: nobody was working at midnight.

**How to tell whether an override is in force.** Read `source` on the admin floor,
which is the single authoritative answer:

```bash
curl -fsS -b "psycho_session=<token>" \
  https://psycho-space.ru/api/game-fintech/admin/layout \
  | jq '{source, installed_at, occupants, id: .layout.id}'
```

- **`daily`** — nobody has touched today's office. It was derived at the last 21:00,
  it is in **no** database row, and `installed_at` is that 21:00 rather than the
  time the process started (which is deliberate: a restart must not look like a
  change). The control room renders it as «ежедневный».
- **`generated`** — an admin pressed «пересобрать» since the last 21:00. There *is*
  a row, and it holds until the next boundary, after which the date's own floor
  takes over again with nobody deleting anything.
- **`starting`** — the hand-made floor a fresh database installs for itself, seen
  only on the first day of a new database (or after a draw failed).

**A restart across the boundary changes nothing**, and that is worth knowing before
suspecting a deploy: the boot path runs the same arithmetic as the tick, so a
process starting at 22:00 derives exactly what a process running through 21:00
derived. There is no catch-up either — three days of downtime install *today's*
office, not four in a row.

**The one thing that genuinely does change the office out from under people is a
deploy that touches `internal/gamefintech/generate.go`.** The day's floor is pure
with respect to a *fixed binary*, so editing the generator retroactively changes
every date's office, including today's — mid-day, with no eviction and no «РЕМОНТ»,
until the next 21:00. Clients refetch the catalogue on an `office_id` they do not
recognise, so it is survivable; it is still the answer to "the room changed and
...
**Then look for one of these six, in this order.** Every flake found so far was one of them —
and note that the first four are defects in the *test*, the fifth is not, and the sixth is
neither, so a test that keeps insisting may be right:
...
6. **…or nothing is racy at all and it is simply the wrong time of day.** The newest kind, and
   the one that reads like nothing else: a suite that is green all afternoon and red at
   **00:00 Moscow / 21:00 UTC**, on the *same commit*. «СИМУЛЯТОР ФИНТЕХА» rebuilds its office
   at that instant and evicts everybody in it ([ADR-062](adrs/ADR-062-the-days-office-is-a-function-of-the-day.md)),
   so any test holding a shift across the boundary reads `renovated` where it expected `left`
   or `promoted`, and any test asserting on the stored floor's provenance sees the day's own
   derived floor instead. It is **not fixable with a flag** — a switch that disabled the
   rebuild would be test-only machinery on a production path, which this project forbids — so:

   - the game's own package tests pin it in the harness (`start()` in
     `internal/gamefintech/service_test.go` sets `installedDay` a day forward, and the
     rotation's own tests set it to the boundary they are driving). Nothing there is exposed.
   - **`test/integration/gamefintech_test.go` and `web/e2e-stack/gamefintech*.spec.ts` run the
     real service against the real clock and are genuinely exposed**, for the seconds a run
     happens to straddle 21:00 UTC. It is a few seconds out of 86 400 per run, it is left
     alone deliberately, and this entry exists so the next person recognises it in ten seconds
     instead of an evening.

   **Diagnose it by the clock, not by the test.** `gh run view <run-id> --json startedAt` — if
   the run spans 21:00:00 UTC and the failing assertion is about a fintech shift's `cause` or
   about a layout's `source`, re-run it and move on.
...
## A full-stack test fails with `rate_limited`, twenty tests away from the cause

**Symptom.** `./dev.sh e2e-stack` goes red on one spec — often a heavy one late in the
run, with two browser contexts — and the failure reads as a timeout plus
`browserContext.close: Protocol error … Failed to find context with id …`. It passes
on its own in seconds. The console attachment holds the actual reason:

> `auth: /me failed ApiError: api error rate_limited (status 429, …)`

**It is the suite's shared allowance, not the spec that failed.** The full-stack suite
runs at `workers: 1` against **one** server, from **one** IP, behind the same per-IP
rate limiter production uses (240/min blanket on `/api`). Every test's
login → navigate → start → quit cycle spends part of that allowance, and the tests do
not fail where the budget is spent — they fail where it runs *out*. So a spec that has
not changed starts failing because three tests were added ahead of it.

**The fix is denser tests, not a raised limit.** The limiter is doing its job, and
relaxing it for the suite would be test-only machinery in a production path — which
[`CLAUDE.md`](../CLAUDE.md) forbids outright. Merge assertions that need the same
office visit into one test: three separate «open the office and look» tests became one
carrying three claims, and the run went from 5.2 min with a failure to 3.5 min clean.

**Before adding a full-stack test, ask what it costs.** A new test is a login, a
navigation, a config fetch and usually a shift — call it ten requests. If the claim can
ride an existing visit, it should. And if a claim only needs the API rather than a
browser (does the draw vary? does the row reach Postgres?), it belongs in
`test/integration/`, where forty shifts cost no browser at all.
...
SELECT (SELECT count(*) FROM wishlist_items    WHERE account_id = '<uuid>') AS ideas,
       ...
       (SELECT count(*) FROM game_vanyadum_visits WHERE account_id = '<uuid>') AS dum_visits,
