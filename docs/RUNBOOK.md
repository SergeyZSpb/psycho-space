...
- **Дядя Ваня answers in RAP** — 2-4 short rhymed bars, one per line, under ~45 characters each so a bar does not wrap on a phone. `TalkStyle` demands rhyme but subordinates it to meaning, and the static `Greeting` is already in verse so the first thing a player sees matches everything after it. Explicit profanity is discouraged in the prompt: that is the class of content which made the provider's filter refuse turns in the first place.
- **Verse survives the round trip, two guards.** The SPA renders the reply with `white-space: pre-line`, and `normalizeVerse` converts literal escape TEXT into real newlines. The second guard is not theoretical: shown the escape sequence in the prompt, the model typed it as two characters, which rendered visibly on screen and wrapped four bars into six lines. The prompt no longer shows it the escape, and the normaliser handles it regardless.
- **The reply bubble scrolls, it does not clip.** It used to be `-webkit-line-clamp: 3` with `overflow: hidden`, which silently ate anything past three lines - unreadable and unreachable. Now `max-height: min(8.5em, 15dvh)` with an internal scroll: the page still never scrolls and the art pane gives up the space. The `dvh` half matters - a fixed `8.5em` overflowed 320x568 by 27px and pushed the OPTIONS off-screen, a worse bug than the one being fixed. Verified with headless Chrome at 320/360/390/414px, before and after a judged turn.
...
- **Model + prompt economics** (`internal/game/llm.go`). Model names come from the endpoint itself: `curl -H "Authorization: Bearer $KEY" $BASE/models` lists e.g. `gpt://<folder>/deepseek-v4-flash/latest`. Prices (Yandex AI Studio, per 1k tokens): **yandexgpt-5-lite 0.2 in / 0.2 out**, **deepseek-v4-flash 0.3 in / 0.5 out / 0.075 cached in**. `modelPrices` keeps input and output separate — a blended rate misreports every turn when output costs 1.67x input — and an unlisted model logs NO `est_cost_rub` rather than a wrong one.
...
- **Per-turn state rides inside the history, not in a re-sent summary.** Each past reply carries a footer `[напряжение: 55; предлагал: a | b | c | d]` (`exchangeFooter`; `Exchange.Anger` + `Exchange.Options` come back from the client). Before, the tail re-derived and re-sent an already-offered list and a recent-replies list every turn — ~1570 tokens of duplication. Measured on real turns: cache hits climbed to **76%** and cost fell from **1.04 to 0.77 RUB/turn (-26%)**.
- **Cost control**: history capped at `historyExchanges` 12 / `historyTokens` 6000 (was effectively ~114 exchanges), so a long game's turns cost the same as turn 12 instead of growing; `maxCompletionTokens` 900 caps the answer, which matters most because output is the dearest rate. One turn ~0.77 RUB on deepseek-v4-flash vs ~0.36 on lite — DeepSeek is ~2x, bought with visibly better play (it produced the first winning run in any audit). The `/attempt` limit is **5/min per IP** (halved from 10 when the model moved to DeepSeek, since a turn costs ~2x there); there is no per-account cap, so one determined player is still the real cost exposure.
...
- **Auditing the option quality**: `options_text` + `already_offered` + `themes_done` are on every `game llm response` line. To replay a judged conversation against the real model, run the app locally with `.env` (the real LLM key) and drive `/api/game/attempt` — the throwaway driver pattern is in the living doc's analysis notes. Beware the **5/min per IP** limit on `/attempt`.
