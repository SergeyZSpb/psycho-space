...
- **Cost control**: history capped at `historyExchanges` 12 / `historyTokens` 6000 (was effectively ~114 exchanges), so a long game's turns cost the same as turn 12 instead of growing; `maxCompletionTokens` 900 caps the answer, which matters most because output is the dearest rate. One turn ~0.77 RUB on deepseek-v4-flash vs ~0.36 on lite — DeepSeek is ~2x, bought with visibly better play (it produced the first winning run in any audit). The `/attempt` limit is 10/min **per IP**; there is no per-account cap, so one determined player is the real cost exposure.
- **Auditing the option quality**: `options_text` + `already_offered` + `themes_done` are on every `game llm response` line. To replay a judged conversation against the real model, run the app locally with `.env` (the real LLM key) and drive `/api/game/attempt` — the throwaway driver pattern is in the living doc's analysis notes. Beware the **10/min per IP** limit on `/attempt`.
