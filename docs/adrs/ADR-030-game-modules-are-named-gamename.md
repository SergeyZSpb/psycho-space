...
Every game module carries its own name at every layer: package `internal/game<name>/`, tables `game_<name>_*`, routes `/api/game-<name>/*`, view `Game<Name>View.vue` served at `/app/game-<name>`, and any exported identifier that names the game from outside its package. Game 1 moved onto the convention from generic `game` naming in this change — `internal/gamekhimki/`, `game_khimki_runs` (`migrations/007_game_khimki_rename.sql`), `/api/game-khimki/*`, `GameKhimkiView.vue` at `/app/game-khimki` — and game 2 is `gamevanyagotchi` throughout. **Game 4 has since moved twice over the same path**: it shipped as `gamekaren` and was reframed to `gamefintech` (`migrations/014_game_fintech_rename.sql`) when the joke stopped being about one man, which is the convention's second full exercise and the reason the rename cost a day rather than a week — `git grep -il gamekaren` was the whole work list. **Shared infrastructure is deliberately not prefixed:** `realtime`, `session`, `account`, `crypto`, `db`, `logging`, `observability`, `httpapi`.
...
_Consequence:_ `game_key` **values** did not move with the table — `smalltalk_khimki` is data, already unambiguous, and the art blobs are keyed on it. Game 4's rename proved the same clause a second time and in the harder direction: its value is still `karen` after the game stopped being called that, and *nothing was uploaded under it*, so the change would have been harmless — which is precisely why it was declined. A rule that holds only when breaking it would hurt is not a rule, and the first blob stored under a quietly-renamed key is the one nobody finds. Neither did the **migration filename**: `013_game_karen.sql` is byte-identical and still named after the old game, because `internal/db/migrate.go` keys `schema_migrations` on the filename — a renamed 013 reads as unapplied, re-runs after 014, and its `CREATE TABLE IF NOT EXISTS game_karen_shifts` quietly recreates an empty old table beside the live one. `/api/game/*` was kept as an alias for exactly one deploy cycle so a stale SPA would not break mid-run; that cycle is over, the registration is deleted, and `TestGameKhimkiLegacyPathAliasIsGone` pins its absence. `/app/game` still redirects permanently, which is a redirect rather than an alias.
