...
sequenceDiagram
    ...
    G-->>B: vanyadum_ready {world_id} — or vanyadum_full when six are already inside
    ...
    loop injected ticker, 20 Hz
        ...
        G->>G: step each occupant in turn — the stable order ROTATED BY THE TICK
...
A different protocol in the same transport, and the one place in this game where **joining happens** — the hello below is the whole of it ([ADR-060](#adr-060--there-is-one-заброшка-nothing-ends-and-a-run-became-a-visit)). There is one заброшка and everybody is in it, but a frame names everybody except its own reader, so nothing here is broadcast: each is built per occupant and addressed to a connection through `PublishTo`, exactly as the office's are.
