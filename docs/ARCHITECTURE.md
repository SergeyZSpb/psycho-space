...
sequenceDiagram
    ...
    loop injected ticker, 20 Hz
        ...
        G->>G: record the tick into the rewind buffer — lag compensation — EVERYBODY, unfiltered
        G->>H: PublishTo(every conn, standings) — once a second, or on the tick the roster changed
        H-->>B: vanyadum_board {b} — a row per occupant {n slot, i pseudonym, s seconds, c? bag}
        G->>H: PublishTo(conn, snapshot) — one PER OCCUPANT, quantised, peers cut to what he can see
        H-->>B: vanyadum_snap {k, ack, x, y, z, yaw, s, hp, pk bitmask, b, d?, r?, c?, ev?, p?}
...
flowchart TB
    ...
    D -- "geometry · textures · input maths · the slot directory — PURE" --> PURE["lib/vanyadum{Level,Texture,Input,Rules,Step,Predict,Interp,Roster,Sound,Flash}.ts"]
...
- **A snapshot is idempotent full state**, so a dropped frame costs nothing and the hub may discard a slow client's backlog. That reaches the peer array too: absence is the whole of leaving the visible set, so there is no join, leave or departure message in either direction. An *event* cannot be expressed that way, which is why `ev` and a peer's `f` ride the snapshot rather than travelling as their own frames: a missed one costs a sound effect or a muzzle flash nobody saw, never a divergence.
...
| What interest management withholds, and what it does not («ВАНЯДУМ») | A snapshot's peers are cut to the reader's own sector and the sectors joined to it by a portal, so where a stranger is standing is not published to somebody who cannot see him. **The pickup mask is not cut**, and that is a stated residual rather than an oversight: a bit clearing names a place and the standings name whose bag grew, so the pair locates a man the reader was never sent. Accepted only while nothing here can be **hit** — the обрез fires at nothing yet — and it is the sentence the iteration that adds damage has to revisit rather than inherit; the fix is to filter the mask per viewer | `internal/gamevanyadum/world.go` — `SnapshotFor`, `canSee`, `heldVisible` |
