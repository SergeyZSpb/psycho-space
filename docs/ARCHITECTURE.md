...
sequenceDiagram
    ...
    loop injected ticker, 20 Hz
        ...
        H-->>B: vanyadum_board {b} — a row per occupant {n slot, i pseudonym, s seconds, c? bag, d? deaths, br? betrayals}
        G->>H: PublishTo(conn, snapshot) — one PER OCCUPANT, quantised, peers AND pickups cut to what he can see
        H-->>B: vanyadum_snap {k, ack, x, y, z, yaw, s, hp, dn?, pr?, pk bitmask, b, d?, r?, c?, ev?, p?}
