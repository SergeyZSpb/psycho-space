...
sequenceDiagram
    ...
    loop injected ticker, 20 Hz
        ...
        G->>G: every command steps the AMPOULE first — its countdown, then the health it has delivered
        G->>G: every command steps the GUN too — both timers down by its dt, then the trigger
        G->>G: a needle in the arm refuses both the trigger and the feet — the angles still land
        ...
        G->>G: a шприц ONLY if he is hurt — a whole man walks over it and leaves it standing
        ...
        G->>G: anything landing on a man takes the needle out — what went in stays in, the rest is lost
        ...
        H-->>B: vanyadum_snap {k, ack, x, y, z, yaw, s, hp, dn? the respawn at hp 0 and the ampoule above it, pr?, pk bitmask, b, d?, r?, c?, ev?, p?, f?}
...
**The шприц is the way back from a barrel, and the whole of it is that it costs time in a building where standing still is the dangerous thing.** It is used by **walking over it**, exactly as the beer is, because there is no use button in this game and there is not going to be one — so the decision it asks for is *when to walk onto it* rather than when to press it, and a man at full health walks over it and leaves it standing, which turns an ampoule into a landmark you take a beating somewhere else and come back for. What it puts back is `SyringeHeal`, **exactly one barrel**, over `SyringeSeconds` — long enough that it is never the cheaper thing to do with somebody in the room (a reload is 1.5 s and gives you something to shoot with), short enough that misjudging it costs about the same again rather than a loading screen, and in the unit that decides it, **about eight metres at `SlopSpeed`**: anything already in the room with you arrives before the plunger is down. For that whole window he can neither walk nor fire, and **only damage stops it** — the trigger and every step are refused, so there is nothing left to interrupt it with, which is deliberate, because the kind rule (tap to cancel and run) makes the window free and therefore not a window at all. Interrupted, **what went in stays in and the rest is lost** with the ampoule, which is lying where it was taken from and comes back on the ordinary respawn. It does not overheal and the time is spent regardless, so injecting at nearly full health is a bad trade rather than a free top-up. The generator changed with it: a building now scatters **one of every kind before a second of anything**, because a uniform draw over two kinds produced a заброшка with no beer — an unreloadable gun — about one time in eight, and one with no medicine just as often, in a match that never ends.

**It reached the wire for nothing, which is what an iteration looks like once the encoding has no slack left.** What the man being injected reads rides **`dn`**, the down timer, with **`hp` as the discriminator** — health at zero means the number is a respawn, above it means the number is an ampoule — and the two are exclusive by construction, since a dead man collects nothing and being hurt is what ends an injection. What everybody else sees rides a **fifth value of `p[].st`**, a field already on every peer on every tick, so the most exploitable state in the game is on every viewer's frame at no cost at all — which is this project's "everybody sees it" rule met by reuse rather than by a field, and at 32 B/s of headroom the only way it could have been met ([ADR-060](#adr-060--there-is-one-заброшка-nothing-ends-and-a-run-became-a-visit)). **It is predicted**, for the same reason the gun is and one more: the browser has to run the same refusal the server is about to run, or it draws a muzzle flash for a trigger that will be ignored and a man walking out of a room he is rooted in. That makes **health a predicted quantity in part** — the heal is inside `Step`, damage is not — and the arithmetic is what keeps the two ends agreeing: how much has gone in is a pure function of the time left, never an accumulator, so a replay from a snapshot lands on the server's number instead of on a sum of the client's own history ([ADR-058](#adr-058--a-predicted-effect-lives-on-player-an-unpredicted-one-lives-on-the-occupant)). The golden vectors carry the countdown and the health, so the port is pinned to the server's arithmetic rather than trusted to match it.
...
flowchart TB
    ...
    D -- "geometry · textures · sprites · input maths · the slot directory — PURE" --> PURE["lib/vanyadum{Level,Texture,Slop,Input,Rules,Step,Predict,Interp,Roster,Sound,Flash,Viewmodel}.ts"]
