...
// TestAHelloStartsAKeyHuntWhenThereIsNotOneAndLeavesOneThatIsRunningAlone is how
// the yard is never without a key, with nothing anywhere holding a timer.
//
// It runs at a hello — a fresh socket, human-paced — and never on the tick, and
// it is the identical insert the winning claim uses for the replacement, against
// the identical partial index. So a cold start and a restart nobody won are one
// mechanism rather than two that have to be kept in agreement. The COUNT is the
// assertion: an implementation that inserted unconditionally and let the index
// swallow the extra would look correct from every frame it produced, and would
// fill the table with refused writes.
func TestAHelloStartsAKeyHuntWhenThereIsNotOneAndLeavesOneThatIsRunningAlone(t *testing.T) {
	kind := mustObjectKind(t, KindKey)
	...
	for _, tc := range []struct {
		...
		{
			name: "a world with no key in it", running: nil, want: 1,
			why: "somebody arriving to a yard nobody has ever played in has to find a hunt to join",
		},
		{
			name: "a hunt already running", running: map[string]string{KindKey: "000000000009-4a1e"}, want: 0,
			why: "every hello would otherwise write a row the index refuses, which is a table full of nothing and a query per visitor",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			repo := playedFor()
			...
			if n := repo.singletonReads(); n != 1 {
				t.Fatalf("one hello asked whether a hunt was running %d times; want exactly 1", n)
			}
			got := repo.insertedObjects()
			if len(got) != tc.want {
				t.Fatalf("%d keys were hidden by one hello into %s; want %d — %s", len(got), tc.name, tc.want, tc.why)
			}
			if tc.want == 0 {
				return
			}
			...
			started := got[0]
			if started.kind != kind.Key {
				t.Errorf("the hunt was started with an object of kind %q; want the catalogue's %q", started.kind, kind.Key)
			}
			if started.locationKey != LocationYard {
				t.Errorf("the key was hidden in %q; want the yard, which is the only place there is", started.locationKey)
			}
			if started.singleton != kind.Singleton {
				t.Errorf("the key was hidden with singleton=%v; want the catalogue's %v, which is what the at-most-one-active index is predicated on",
					started.singleton, kind.Singleton)
			}
			if started.owner != "" {
				t.Errorf("the key was hidden by %q; the world loses it, and nobody in particular owns a hunt", started.owner)
			}
			if started.expires != nil {
				t.Errorf("the key expires at %s; a hunt ends when somebody wins it", started.expires.UTC())
			}
			if started.at.X < 0 || started.at.X > 1 || started.at.Y < 0 || started.at.Y > 1 {
				t.Errorf("the key was hidden at (%v,%v), off the plane; the column itself CHECKs 0..1, so that insert would be refused outright",
					started.at.X, started.at.Y)
			}
		})
	}
}
