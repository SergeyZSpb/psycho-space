...
// `Step` now exist twice — in Go and in TypeScript (web/src/lib/karenStep.ts).
...
func buildGolden() goldenFile {
	f := goldenFile{
		Note: "Generated by TestGoldenVectors in internal/gamekaren. " +
			"Pins the TypeScript port (web/src/lib/karenStep.ts) to the Go original. " +
			...
			"Regenerate with: ./dev.sh test -run TestGoldenVectors ./internal/gamekaren/ -update",
...
func TestGoldenVectors(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("%v — generate it with: ./dev.sh test -run TestGoldenVectors ./internal/gamekaren/ -update", err)
	}
	var want goldenFile
