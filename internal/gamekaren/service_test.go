...
func TestHelloIsAnsweredWithTheShiftYouAreOn(t *testing.T) {
	...
	h.svc.HandleInbound(context.Background(), m, Room, []byte(`{"t":"karen_hello"}`))

	ready := h.tr.framesOfType(TypeReady)
	if len(ready) != 1 {
		t.Fatalf("expected one ready frame, got %d", len(ready))
	}
	if ready[0].Frame["shift_id"] != shiftID {
		t.Fatalf("ready names shift %v, StartShift returned %v", ready[0].Frame["shift_id"], shiftID)
	}
}
...
func TestInputMovesThePlayerAndTheSnapshotSaysSo(t *testing.T) {
	...
	for i := 1; i <= 8; i++ {
		h.svc.HandleInbound(context.Background(), m, Room,
			[]byte(`{"t":"karen_input","k":0,"cmds":[{"q":`+itoa(i)+`,"dt":0.05,"mx":1}]}`))
		h.step(t)
	}
...
func TestConfigIsTheCatalogue(t *testing.T) {
	h := start(t)
	if got := h.svc.Config(); got.GameKey != GameKey || len(got.Office.Desks) != len(Desks) {
