...
func (PostgresRepository) InsertShift(ctx context.Context, q db.DBTX, s Shift) error {
	_, err := q.Exec(ctx,
		`INSERT INTO game_karen_shifts (id, account_id, cause, salary, seconds)
		 VALUES ($1, $2, $3, $4, $5)`,
		s.ID, s.AccountID, s.Cause, s.Salary, s.Seconds)
	if err != nil {
		return fmt.Errorf("gamekaren: insert shift: %w", err)
	}
	return nil
}
...
func (PostgresRepository) RecentShifts(ctx context.Context, q db.DBTX, accountID uuid.UUID, limit int) ([]Shift, error) {
	rows, err := q.Query(ctx,
		`SELECT `+shiftColumns+`
		   FROM game_karen_shifts
		  WHERE account_id = $1 AND deleted_at IS NULL
		  ORDER BY created_at DESC
		  LIMIT $2`,
		accountID, limit)
	if err != nil {
		return nil, fmt.Errorf("gamekaren: recent shifts: %w", err)
	}
...
func (PostgresRepository) TopShifts(ctx context.Context, q db.DBTX, limit int) ([]Shift, error) {
	rows, err := q.Query(ctx,
		...
		limit)
	if err != nil {
		return nil, fmt.Errorf("gamekaren: top shifts: %w", err)
	}
...
func scanShifts(rows pgx.Rows, limit int, what string) ([]Shift, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.AccountID, &s.Cause, &s.Salary, &s.Seconds, &s.CreatedAt); err != nil {
			return nil, fmt.Errorf("gamekaren: scan %s shift: %w", what, err)
		}
		out = append(out, s)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("gamekaren: iterate %s shifts: %w", what, err)
	}
	return out, nil
}
