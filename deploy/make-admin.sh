#!/usr/bin/env bash
# Promote the account whose vk_user_ref hex starts with <handle> to admin+approved.
# Installed at /usr/local/bin/make-admin by bootstrap.sh; run as: sudo make-admin <handle>
set -euo pipefail
HANDLE="${1:?usage: make-admin <handle-hex-prefix (shown on the pending screen)>}"
case "$HANDLE" in
