...
- **No assistant session links in a commit message.** A `Claude-Session: https://claude.ai/code/session_…` trailer is the assistant's default, and it does not belong in this repository's history: the URL opens for exactly one account, so it is unreadable noise for every other reader of a **public** log, and it publishes an identifier tied to the owner's workspace for no benefit. It is **not** a credential and leaking one is not an incident — which is precisely why the fix is to stop adding them rather than to rewrite history: the twenty already in the log stay, because rewriting them would break every commit SHA anybody has ever linked and force-push a branch that auto-deploys, for a cosmetic gain. `Co-Authored-By:` is fine and stays — it is an attribution, and it resolves for everyone.
