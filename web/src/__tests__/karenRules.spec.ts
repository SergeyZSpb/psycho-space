...
import { KAREN_LORE, KAREN_PROSE, buildRules, endingFor } from '../lib/karenRules';
...
const allText = (c: KarenConfig | null) =>
  buildRules(c)
    .flatMap((b) => b.lines)
...
describe('buildRules', () => {
  ...
  it('marks the hand-written block, and only that one', () => {
    ...
    expect(prose[0].lines).toBe(KAREN_PROSE);
  });
  ...
  it('still says how to play when the catalogue says nothing at all', () => {
    ...
    const blocks = buildRules({} as unknown as KarenConfig);
...
describe('the hand-written copy', () => {
  ...
  it('sets up the joke before the numbers do', () => {
    expect(KAREN_LORE).toContain('не работать и получать за это деньги');
    expect(KAREN_LORE).toContain('лысый');
  });
});
