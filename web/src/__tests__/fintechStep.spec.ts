...
const VECTORS = resolve(__dirname, '../../../internal/gamefintech/testdata/step_vectors.json');
 ...
 *
 * WHICH MAKES THE FALLBACK NUMBERS PART OF THE SAFETY NET, AND THEY HAD ROTTED.
 * Every one of them was the value it had on the day this file was written, and
 * six had since been retuned — a 3.2 m/s walk against today's 6.4, a 9 m/s dash
 * against 20, a 4 s cooldown against 2.2, a 6 s ramp against 3.5, a 0.3 s grace
 * against 0.18, a 0.22 s dash against 0.26. Nothing noticed, because the file is
 * only read when the path is broken, and the path was not broken until this
 * rename moved it. So the guard test below was the only thing standing between a
 * moved vectors file and a suite quietly asserting the rules of a game that
 * stopped existing months ago.
 *
 * KEEP THESE IN STEP WITH internal/gamefintech/content.go. They are a duplicate
 * of the catalogue and the only defence against them rotting again is that the
 * numbers are also on screen: they are the ones the splash cheatsheet derives.
 */
...
describe('the TypeScript port of Step conforms to the Go original', () => {
  ...
  it('agrees on the constants it is simulating with', () => {
    ...
    // IDLE_THRESHOLD in fintechStep.ts. If the server retunes it, this fails.
...
const ROOM: readonly FintechRect[] = [];
...
describe('the office', () => {
  ...
  it('never leaves anybody standing inside a desk', () => {
    const desks: FintechRect[] = [{ x: 4, y: 8, w: 4, h: 2 }];
